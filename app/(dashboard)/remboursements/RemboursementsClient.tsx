...
export default function RemboursementsClient({ factures }: { factures: any[] }) {
    ...
    <div>
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h2 className="text-2xl font-bold font-heading text-foreground">Suivi des Remboursements</h2>
          <p className="mt-2 text-sm text-foreground-muted">
            Enregistrez les paiements en numéraire et les pesées pour les remboursements en nature.
          </p>
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className={`mt-2 text-sm font-semibold ${reste < 0 ? 'text-primary' : 'text-danger'}`}>
                    {reste < 0 ? 'Surplus de remboursement : ' : 'Reste : '}
                    {Math.abs(reste).toLocaleString('fr-FR', { maximumFractionDigits: 0 })} FCFA
                  </div>
                </button>
              )
            })}
          ...
          {selectedFacture ? (
            <div className="bg-surface border border-surface-border rounded-lg shadow-sm p-6 h-full">
              <h3 className="text-xl font-bold text-foreground mb-6">
                Facture de {selectedFacture.membre?.prenom} {selectedFacture.membre?.nom}
              </h3>
              
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-background rounded-md p-4 border border-surface-border">
                  <div className="text-sm text-foreground-muted">Total Dû</div>
                  <div className="text-xl font-bold text-foreground">{selectedFacture.montant_total.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} FCFA</div>
                </div>
                <div className="bg-background rounded-md p-4 border border-surface-border">
                  {(() => {
                    ...
                    return r < 0 ? (
                      <>
                        <div className="text-sm text-foreground-muted">Surplus de Remboursement</div>
                        <div className="text-xl font-bold text-primary">
                          {Math.abs(r).toLocaleString('fr-FR', { maximumFractionDigits: 0 })} FCFA
                        </div>
                      </>
                    ) : (
                      <>
                        <div className="text-sm text-foreground-muted">Reste à Payer</div>
                        <div className="text-xl font-bold text-danger">
                          {r.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} FCFA
                        </div>
                      </>
                    )
                  })()}
              ...
              <div className="space-y-4">
                <h4 className="font-medium text-foreground">Enregistrer un paiement</h4>
                <div className="flex gap-4">
                  ...
                  >
                    ...
                    <span className="font-semibold text-foreground">En Numéraire (Espèces)</span>
                  </button>
                  <button 
                  ...
                  >
                    ...
                    <span className="font-semibold text-foreground">En Nature (Pesée)</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="h-full flex items-center justify-center border-2 border-dashed border-surface-border rounded-lg bg-surface">
              <span className="text-foreground-muted">Sélectionnez un membre pour voir ses remboursements</span>
            </div>
          )}
        </div>
      </div>
