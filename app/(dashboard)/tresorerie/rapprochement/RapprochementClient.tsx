...
export default function RapprochementClient({ campagnes }: { campagnes: any[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="max-w-sm mb-8">
        <label htmlFor="campagne" className="block text-sm font-medium text-foreground mb-1">Choisir une campagne</label>
        <select
          id="campagne"
      ...
      {loading && <div className="text-foreground-muted animate-pulse">Chargement du bilan...</div>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-surface border border-surface-border p-4 rounded-lg shadow-sm">
              ...
              <div className="text-2xl font-bold text-foreground">
                {data.bilan.montantAccorde.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} <span className="text-sm font-normal text-foreground-muted">FCFA</span>
              </div>
            ...
            <div className="bg-surface border border-surface-border p-4 rounded-lg shadow-sm">
              ...
              <div className="text-2xl font-bold text-foreground">
                {data.bilan.totalPaiementsFournisseurs.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} <span className="text-sm font-normal text-foreground-muted">FCFA</span>
              </div>
            </div>
            ...
            <div className="bg-surface border border-surface-border p-4 rounded-lg shadow-sm">
              ...
              <div className="text-2xl font-bold text-foreground">
                {data.bilan.totalRetraits.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} <span className="text-sm font-normal text-foreground-muted">FCFA</span>
              </div>
            </div>
            ...
            <div className={`border p-4 rounded-lg shadow-sm text-white ${data.bilan.soldeCredit >= 0 ? 'bg-success border-success' : 'bg-danger border-danger'}`}>
              <div className="flex items-center gap-2 font-semibold mb-2 text-white/80">
                <CheckCircle2 className="w-5 h-5" />
                Solde du Crédit
              </div>
              <div className="text-2xl font-bold">
                {data.bilan.soldeCredit.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} <span className="text-sm font-normal text-white/80">FCFA</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-surface rounded-lg shadow border border-surface-border">
              <div className="px-4 py-5 sm:px-6 border-b border-surface-border">
                <h3 className="text-lg font-medium leading-6 text-foreground">Paiements Fournisseurs</h3>
              </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="ml-2 flex flex-shrink-0">
                        <p className="inline-flex rounded-full bg-warning/10 px-2 text-xs font-semibold leading-5 text-warning">
                          {p.montant.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} FCFA
                        </p>
                      </div>
                    </div>
                    <div className="mt-2 sm:flex sm:justify-between">
                      <div className="sm:flex">
                        <p className="flex items-center text-sm text-foreground-muted">
                          Compte : {p.comptes?.nom || 'Inconnu'}
                        </p>
                      </div>
                      <div className="mt-2 flex items-center text-sm text-foreground-muted sm:mt-0">
                        {new Date(p.date_transaction).toLocaleDateString('fr-FR')}
                      </div>
                    </div>
                  </li>
                ))}
                {data.paiementsFournisseurs.length === 0 && (
                  <li className="px-4 py-4 sm:px-6 text-sm text-foreground-muted italic">Aucun paiement.</li>
                )}
              </ul>
            </div>
            ...
            <div className="bg-surface rounded-lg shadow border border-surface-border">
              <div className="px-4 py-5 sm:px-6 border-b border-surface-border">
                <h3 className="text-lg font-medium leading-6 text-foreground">Retraits en Espèces</h3>
              </div>
              <ul role="list" className="divide-y divide-surface-border">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="ml-2 flex flex-shrink-0">
                        <p className="inline-flex rounded-full bg-danger/10 px-2 text-xs font-semibold leading-5 text-danger">
                          {r.montant.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} FCFA
                        </p>
                      </div>
                    </div>
                    <div className="mt-2 sm:flex sm:justify-between">
                      <div className="sm:flex">
                        <p className="flex items-center text-sm text-foreground-muted">
                          Compte : {r.comptes?.nom || 'Inconnu'}
                        </p>
                      </div>
                      <div className="mt-2 flex items-center text-sm text-foreground-muted sm:mt-0">
                        {new Date(r.date_transaction).toLocaleDateString('fr-FR')}
                      </div>
                    </div>
                  </li>
                ))}
                {data.retraits.length === 0 && (
                  <li className="px-4 py-4 sm:px-6 text-sm text-foreground-muted italic">Aucun retrait.</li>
                )}
              </ul>
            </div>
          </div>
