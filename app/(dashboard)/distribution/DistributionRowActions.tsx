...
export default function DistributionRowActions({ id, intrantNom, quantite }: { id: string, intrantNom: string, quantite: number }) {
  ...
  return (
    <>
      <div className="flex justify-end gap-2">
        <button onClick={() => setIsEditOpen(true)} className="text-secondary hover:text-secondary/80 p-1" title="Modifier la quantité">
          <Edit className="h-4 w-4" />
        </button>
        <button onClick={() => setIsDeleteOpen(true)} className="text-foreground-muted hover:text-danger p-1" title="Annuler la distribution">
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-surface text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-surface-border">
              <div className="bg-surface px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <h3 className="text-lg font-semibold leading-6 text-foreground mb-4">
                  Modifier la quantité distribuée
                </h3>
                <p className="text-sm text-foreground-muted mb-4">
                  Produit : <strong>{intrantNom}</strong><br/>
                  Quantité actuelle : <strong>{quantite}</strong>
                </p>
                <form id={`edit-dist-${id}`} onSubmit={handleEdit}>
                  <label className="block text-sm font-medium text-foreground mb-1">Nouvelle quantité</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0.01"
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-surface text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-surface-border">
              <div className="bg-surface px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                ...
                <p className="text-sm text-foreground-muted">
                  Êtes-vous sûr de vouloir annuler cette remise de <strong>{quantite} {intrantNom}</strong> ?
                  <br/><br/>
                  <span className="text-success">✅ Le stock sera recrédité de la même quantité.</span>
                </p>
              </div>
              <div className="bg-background/50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                <button
                ...
                >
                  {loading ? 'Annulation...' : 'Annuler la distribution'}
                </button>
                <button
                  type="button"
