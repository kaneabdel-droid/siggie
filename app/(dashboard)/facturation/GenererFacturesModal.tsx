...
export default function GenererFacturesModal() {
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-surface text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-surface-border">
              <div className="bg-surface px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <h3 className="text-lg font-semibold leading-6 text-foreground mb-4">
                  Générer les factures de la campagne
                </h3>
                <p className="text-sm text-foreground-muted mb-4">
                  Cette action va additionner les intrants reçus et les crédits validés pour chaque membre, et générer ou mettre à jour les factures.
                </p>
                <form id="generer-factures-form" onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label htmlFor="campagne_id" className="block text-sm font-medium text-foreground">Campagne</label>
                    <select
                      name="campagne_id"
                      id="campagne_id"
                      required
